...
SINGLE_BATTLE_TEST("Shield Dust blocks powder and spore moves (Gen6+)")
{
    u32 gen = 0;
    PARAMETRIZE { gen = GEN_5; }
    PARAMETRIZE { gen = GEN_6; }
...
DOUBLE_BATTLE_TEST("Shield Dust blocks damage from sandstorm")
{
    GIVEN {
        PLAYER(SPECIES_WYNAUT)    { Speed(50); }
        PLAYER(SPECIES_HELIOLISK) { Speed(40); Ability(ABILITY_SAND_VEIL); }
        OPPONENT(SPECIES_PINECO)  { Speed(30); Ability(ABILITY_SHIELD_DUST); }
        OPPONENT(SPECIES_STARLY)     { Speed(20); }
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_SANDSTORM); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Shield Dust blocks Effect Spore's effect")
{
    GIVEN {
        PLAYER(SPECIES_PINECO) {Ability(ABILITY_SHIELD_DUST);}
        OPPONENT(SPECIES_SHROOMISH) {Ability(ABILITY_EFFECT_SPORE);}
    } WHEN {
        TURN { MOVE(player, MOVE_TACKLE, WITH_RNG(RNG_EFFECT_SPORE, 1)); }
    } SCENE {
