...
DOUBLE_BATTLE_TEST("Fermata causes partner's sound moves to hit twice")
{
    GIVEN {
        ...
        TURN {MOVE(playerLeft, MOVE_CELEBRATE); MOVE(playerRight, MOVE_ROUND, target: opponentLeft); MOVE(opponentLeft, MOVE_HOWL); MOVE(opponentRight, MOVE_ROUND, target: playerRight);}
    } SCENE {
        ...
        HP_BAR(playerRight);
        ANIMATION(ANIM_TYPE_MOVE, MOVE_HOWL, opponentLeft);
}
}
